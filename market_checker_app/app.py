...
def _format_strength(value: object) -> str:
    raw = str(value) if value is not None else ""
    return SIGNAL_STRENGTH_LABELS.get(raw, raw)
...
if st.session_state.last_result:
    ...
    with tab_signals:
        st.caption("Signal = směr doporučení (BUY/HOLD/SELL). Síla signálu = jistota/intenzita (slabý/střední/silný/velmi silný).")
        ...
        signals_display = signals_df[[column for column in display_columns if column in signals_df.columns]].copy()
        rename_map = {
            "signal": "doporučení",
            "signal_strength": "síla_signálu",
        }
        signals_display = signals_display.rename(columns=rename_map)
        if "síla_signálu" in signals_display.columns:
            signals_display["síla_signálu"] = signals_display["síla_signálu"].map(_format_strength)

        st.dataframe(signals_display, width="stretch")
