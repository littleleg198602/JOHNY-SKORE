...
def _render_dashboard(signals_df: pd.DataFrame, ranking_tables: dict[str, pd.DataFrame], dashboard_tables: dict[str, pd.DataFrame]) -> None:
    ...
    st.markdown("### Kalibrační analýza HOLD neutrality")
    calibration = VisualizationService.prepare_calibration_analysis(
        signals_df,
        app_cfg.decision_weights,
        app_cfg.decision_thresholds,
    )
    st.subheader("Top HOLD tickery (diagnostika)")
    st.dataframe(calibration.get("hold_top_tickers", pd.DataFrame()), width="stretch")

    cal_col1, cal_col2 = st.columns(2)
    with cal_col1:
        st.subheader("HOLD concentration analýza")
        st.dataframe(calibration.get("hold_concentration", pd.DataFrame()), width="stretch")
    with cal_col2:
        st.subheader("Sensitivity simulace hold_band")
        st.dataframe(calibration.get("hold_band_sensitivity", pd.DataFrame()), width="stretch")

    cal_col3, cal_col4 = st.columns(2)
    with cal_col3:
        st.subheader("Confidence sanity check")
        st.dataframe(calibration.get("confidence_sanity", pd.DataFrame()), width="stretch")
    with cal_col4:
        st.subheader("Technical-driver effectiveness")
        st.dataframe(calibration.get("technical_driver_effectiveness", pd.DataFrame()), width="stretch")
