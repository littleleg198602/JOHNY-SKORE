...
from market_checker_app.analysis.scoring import _decision_from_modules
from market_checker_app.config import DecisionModuleWeights, DecisionThresholds
from market_checker_app.models import ModuleAxisResult
...
class VisualizationService:
    ...
    @staticmethod
    def prepare_calibration_analysis(
        signals_df: pd.DataFrame,
        decision_weights: DecisionModuleWeights,
        decision_thresholds: DecisionThresholds,
        hold_bands: list[float] | None = None,
    ) -> dict[str, pd.DataFrame]:
        if signals_df.empty:
            empty = pd.DataFrame()
            return {
                "hold_top_tickers": empty,
                "hold_concentration": empty,
                "hold_band_sensitivity": empty,
                "confidence_sanity": empty,
                "technical_driver_effectiveness": empty,
            }

        frame = signals_df.copy()
        for col in ["bull_score", "bear_score", "bull_bear_spread", "final_confidence", "risk_score"]:
            if col in frame.columns:
                frame[col] = pd.to_numeric(frame[col], errors="coerce")

        hold_mask = frame.get("signal", pd.Series(dtype=str)).fillna("") == "HOLD"
        hold_df = frame[hold_mask].copy()

        hold_top = hold_df[[c for c in ["ticker", "bull_score", "bear_score", "bull_bear_spread", "final_confidence", "primary_driver", "module_directions", "blocked_reasons"] if c in hold_df.columns]].copy()
        hold_top = hold_top.rename(columns={"bull_bear_spread": "spread", "final_confidence": "overall_confidence"})
        if "spread" in hold_top.columns:
            hold_top["abs_spread"] = hold_top["spread"].abs()
            hold_top = hold_top.sort_values(["abs_spread", "overall_confidence"], ascending=[True, False]).drop(columns=["abs_spread"]).head(30)

        blocked_series = hold_df.get("blocked_reasons", pd.Series(dtype=str)).fillna("").astype(str)
        module_series = hold_df.get("module_directions", pd.Series(dtype=str)).fillna("").astype(str)
        small_spread_count = int(hold_df.get("bull_bear_spread", pd.Series(dtype=float)).abs().le(decision_thresholds.hold_band).sum())
        panic_block_count = int(blocked_series.str.contains("panic_", regex=False).sum())
        low_conf_count = int(blocked_series.str.contains("low_confidence", regex=False).sum())
        tech_news_conflict_count = 0
        mixed_neutral_count = 0
        for raw in module_series.tolist():
            mod = VisualizationService._parse_json_dict(raw)
            tech_dir = str(mod.get("technical", ""))
            news_dir = str(mod.get("news", ""))
            all_dirs = [str(v) for v in mod.values()]
            if tech_dir in {"bullish", "bearish"} and news_dir in {"bullish", "bearish"} and tech_dir != news_dir:
                tech_news_conflict_count += 1
            if all_dirs.count("neutral") >= 2:
                mixed_neutral_count += 1
        hold_concentration = pd.DataFrame(
            [
                {"cause": "small_spread", "count": small_spread_count},
                {"cause": "technical_news_conflict", "count": tech_news_conflict_count},
                {"cause": "panic_block", "count": panic_block_count},
                {"cause": "low_confidence", "count": low_conf_count},
                {"cause": "mixed_neutral_modules", "count": mixed_neutral_count},
            ]
        )
        hold_total = max(1, len(hold_df))
        hold_concentration["share_pct"] = (hold_concentration["count"] / hold_total * 100).round(2)
        hold_concentration = hold_concentration.sort_values("count", ascending=False)

        # Sensitivity simulation (only analysis; no production config change)
        if hold_bands is None:
            hold_bands = [max(1.0, decision_thresholds.hold_band - 3), decision_thresholds.hold_band, decision_thresholds.hold_band + 3]
        simulation_rows: list[dict[str, object]] = []
        for hb in hold_bands:
            simulated_counts = {"STRONG BUY": 0, "BUY": 0, "HOLD": 0, "SELL": 0, "STRONG SELL": 0}
            thresholds = replace(decision_thresholds, hold_band=float(hb))
            for row in frame.to_dict(orient="records"):
                raw_mods = VisualizationService._parse_json_list(row.get("module_breakdown"))
                    ...
                    try:
                        modules.append(
                            ModuleAxisResult(
                                ...
                                bull_contribution=float(item.get("bull_contribution", 50.0)),
                                bear_contribution=float(item.get("bear_contribution", 50.0)),
                ...
                if len(modules) != 4:
                    continue
                signal, *_ = _decision_from_modules(modules, float(row.get("risk_score") or 0.0), decision_weights, thresholds)
                simulated_counts[signal] = simulated_counts.get(signal, 0) + 1
            simulation_rows.append({"hold_band": hb, **simulated_counts})
        hold_band_sensitivity = pd.DataFrame(simulation_rows)

        high_conf_hold = hold_df[hold_df.get("final_confidence", pd.Series(dtype=float)).ge(70)]
        confidence_sanity = pd.DataFrame(
            [
                {"metric": "hold_total", "value": len(hold_df)},
                {"metric": "hold_high_confidence_>=70", "value": len(high_conf_hold)},
                {"metric": "hold_high_confidence_share_pct", "value": round((len(high_conf_hold) / hold_total) * 100, 2)},
                {
                    "metric": "interpretation",
                    "value": "mixed certainty (spread/konflikt)" if small_spread_count + tech_news_conflict_count >= low_conf_count else "possible confidence inflation",
                },
            ]
        )

        tech_bull_total = tech_bull_hold = 0
        tech_bear_total = tech_bear_hold = 0
        for _, row in frame.iterrows():
            mod = VisualizationService._parse_json_dict(row.get("module_directions"))
            tech_dir = str(mod.get("technical", ""))
            ...
            if tech_dir == "bullish":
                ...
                if signal == "HOLD":
                    tech_bear_hold += 1
        technical_driver_effectiveness = pd.DataFrame(
            [
                {
                    "bucket": "technical_bullish",
                    "total": tech_bull_total,
                    "hold_count": tech_bull_hold,
                    "hold_share_pct": round((tech_bull_hold / max(1, tech_bull_total)) * 100, 2),
                },
                {
                    "bucket": "technical_bearish",
                    "total": tech_bear_total,
                    "hold_count": tech_bear_hold,
                    "hold_share_pct": round((tech_bear_hold / max(1, tech_bear_total)) * 100, 2),
                },
            ]
        )

        return {
            "hold_top_tickers": hold_top,
            "hold_concentration": hold_concentration,
            "hold_band_sensitivity": hold_band_sensitivity,
            "confidence_sanity": confidence_sanity,
            "technical_driver_effectiveness": technical_driver_effectiveness,
        }
