...
class FullUniverseContractTests(unittest.TestCase):
    def test_exactly_687_unique_tickers_complete_with_cached_yahoo(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue((signals["current_price_status"] == "USABLE").all())
            self.assertTrue((signals["technical_status"] == "USABLE").all())
            self.assertTrue((signals["ranking_status"] == "USABLE").all())
            self.assertEqual(687, int(signals["ranking_eligible"].sum()))
            self.assertTrue(signals["current_price_fetched_at"].notna().all())
